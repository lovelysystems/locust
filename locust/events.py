...
def add_distributed_hook(name):
    global DISTRIBUTED_HOOKS
    if name not in DISTRIBUTED_HOOKS:
        DISTRIBUTED_HOOKS[name] = EventHook()


def get_distributed_hook(name, default=None):
    global DISTRIBUTED_HOOKS
    return DISTRIBUTED_HOOKS.get(name, default)
